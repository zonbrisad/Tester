...
void CHANNEL_Update(CHANNEL* chn, CHANNEL_VAL newValue, uint8_t divider) {
    ...
    switch (chn->mode) {
        ...
        case CHANNEL_MODE_MAX:
            ...
            if (nVal > chn->value) chn->value = nVal;
            break;
...
char* CHANNEL_toString(CHANNEL* chn) {
    ...
    //CHANNEL_modeToString(chn->mode), CHANNEL_GetValue(chn), int2bin(ibuf,
    //chn->flags,8));
    sprintf(buf,
            " %2d   %-10s  " E_BR_MAGENTA "%-16s" E_RESET " %-10s" E_CYAN
            "%10d" E_RESET "   %08b",
            chn->rid, CHANNEL_get_id(chn), chn->name,
            CHANNEL_modeToString(chn->mode), CHANNEL_GetValue(chn), chn->flags);
    return buf;
}
