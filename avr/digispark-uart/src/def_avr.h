...
#ifdef ARDUINO
#define BOARD_LED_PIN B,5
#endif

#ifdef ARDUINO_MEGA
#define BOARD_LED_PIN B, 7
#endif

#ifdef DIGISPARK
#define BOARD_LED_PIN B, 1
#endif


inline void BOARD_LED_INIT(void)   { gpio_init(BOARD_LED_PIN, 1, 0); }
inline void BOARD_LED_SET(bool on) { gpio_write(BOARD_LED_PIN, on); }
inline void BOARD_LED_ON(void)     { gpio_write(BOARD_LED_PIN, 1); }
inline void BOARD_LED_OFF(void)    { gpio_write(BOARD_LED_PIN, 0); }
inline void BOARD_LED_TOGGLE(void) { gpio_toggle(BOARD_LED_PIN); }
inline bool BOARD_LED_IS_ON(void)  { return gpio_read(BOARD_LED_PIN); }
